<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="utf-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no" />
    <meta name="description" content="" />
    <meta name="author" content="" />

    <title>AECServer | <?= $title ?></title>

    <!-- Custom fonts for this template-->
    <link href="<?= base_url('') ?>/assets/dist/vendor/fontawesome-free/css/all.min.css" rel="stylesheet" type="text/css" />
    <link href="https://fonts.googleapis.com/css?family=Nunito:200,200i,300,300i,400,400i,600,600i,700,700i,800,800i,900,900i" rel="stylesheet" />

    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous" />

    <!-- Custom styles for this template-->
    <link href="<?= base_url('') ?>/assets/dist/css/sb-admin-2.min.css" rel="stylesheet" />

    <link rel="stylesheet" href="<?= base_url('') ?>/assets/dist/css/style.css">

    <link href="<?= base_url('') ?>/assets/dist/vendor/datatables/dataTables.bootstrap4.min.css" rel="stylesheet" />

    <link href="<?= base_url('assets/dist/vendor/bootstrap/css/bootstrap.min.css'); ?>" rel="stylesheet">
    <link href="<?= base_url('assets/dist/vendor/fontawesome-free/css/all.min.css'); ?>" rel="stylesheet">
    <link href="<?= base_url('assets/dist/css/sb-admin-2.min.css'); ?>" rel="stylesheet">
    <script src="<?= base_url('assets/dist/vendor/jquery/jquery.min.js'); ?>" rel="stylesheet"></script>
    <script src="<?= base_url('assets/dist/vendor/bootstrap/js/bootstrap.bundle.min.js'); ?>" rel="stylesheet"></script>
    <script src="<?= base_url('assets/dist/vendor/chart.js/Chart.min.js'); ?>" rel="stylesheet"></script>
</head>

<body id="page-top">